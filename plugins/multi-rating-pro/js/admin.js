jQuery(document).ready(function() {	
	...
	jQuery("#add-new-rating-form-btn").click(function() {
		jQuery("#form-submitted").val("true");
	});
	...
	var rowActions = jQuery("#rating-form-table-form .row-actions > a");
	jQuery.each(rowActions, function(index, element) {
		jQuery(element).click(function(event) { 
			var btnId = this.id;
			var parts = btnId.split("-"); 
			...
			if (action === "edit") {
				...
				// save
				var value = jQuery("#field-" + column + "-" + rowId).val();
				var data =  { 
						action : "save_rating_form_table_column",
						nonce : mrp_admin_data.ajax_nonce,
						column : column,
						ratingFormId : rowId,
						value : value
